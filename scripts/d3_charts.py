...
from ocr_bench.metrics.base import Aggregate  # noqa: E402
...
engine không có cột vẫn phải in được lý do — xem `_nhan_ngan`."""
LE_TRAI = 190
...
def _nhan_ngan(a: Aggregate) -> str:
...
def _o(a: Aggregate) -> tuple[float | None, str]:
...
def _svg_mot_nhom(
    ...
    cao_thanh = max(3, (CAO_DONG - 8) // len(engines))
    ...
    d: list[str] = [
        ...
        f'<text x="12" y="22" font-size="14" font-weight="600">'
        f"{_thoat(' × '.join(engines))}</text>",
        f'<text x="12" y="40" font-size="11" fill="#475569">'
    ...
    for i, e in enumerate(engines):
        ...
        d.append(f'<text x="{x + 14}" y="59" font-size="11">{_thoat(e)}</text>')
        x += 20 + 7 * len(e)
    ...
    for r, m in enumerate(metrics):
        ...
        d.append(
            ...
            f'text-anchor="end" fill="#0f172a">{_thoat(m)}</text>'
        )
        oo = [bang.cell(m, e) for e in engines]
        o = [(e, a, *_o(a)) for e, a in zip(engines, oo)]
        if all(gt is None for *_x, gt, _n in o):
            ...
            d.append(
                ...
                f'font-size="10" fill="#94a3b8">{_thoat(o[0][3])}</text>'
            )
            continue
