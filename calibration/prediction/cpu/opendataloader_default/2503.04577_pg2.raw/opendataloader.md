2 Alireza Kabgani, Masoud Ahookhosh
...
The proximal operator enjoys several favorable properties under convexity, contributing to its popularity for developing numerical methods: (i) proxγφ is single-valued at each x ∈ Rn; (ii) it is Lipschitz continuous with the constant 1; (iii) it satisfies the identity
...
where the latter set denotes the set of fixed points of proxγφ; see, e.g., [8, Chapter 12]. The generic proximalpoint method is given by xk+1 = proxγφ(xk), satisfying xk → x∗ ∈ arg minx∈Rnφ(x) under the convexity. For convex φ, it holds that

proxγφ(xk) = xk − γ∇φγ(xk). (1.3)
