...
# arXiv:2503.05989v1 [eess.SY] 7 Mar 2025
...
where x ∈ Rn is the state, u ∈ R and y ∈ R are the input and output respectively, f(·) : Rn → Rn and g(·) : Rn → Rn are vector fields, f(0) = 0 - the origin is an equilibrium of the autonomous system - and h(·) : Rn → R is the output function.

Definition 1: Dissipativity and Passivity. Let ω(·,·) : R × R → R be an integrable function, called the supply rate. A system in the form (1)-(2) is said to be dissipative with supply rate ω(u(t),y(t)) if there exists a function S(·) : Rn → R satisfying, in some domain of the state
