...
If the error is less than 90 (the glucose is less than the reference), the reward will be given by the linear function: (1 − | e | ) / 20 If the error is greater than 90 (the glucose is greater than

If the error is greater than 90 (the glucose is than the   reference) ,  the reward will be given by the linear function: (1 _ |el)/70 greater

reference), the reward will be given by the function, which at e = 0 gives the value 20, and at e = 10 gives 1

1 . 262 · ( | e | ) + 2 We then apply penalties to our agent as follows: − 2 × 10 − 6 · i − 10 − 6 · c , where the first term is introduced to mitigate oscillations in the blood glucose signal, ensuring stability, and the second term is designed to optimize the injected insulin, discouraging excessive usage. These penalties contribute to fine-tuning the agent’s behavior for more effective and stable blood glucose regulation.
...
well as the were systematically   carried out   using Simulink.
